...
int LightContributionToSample( trace_t *trace )
{
	...
	float			add;
	float			dist;
	float			addDeluxe, addDeluxeBounceScale = 0.25f;
	...
	if( light->type == EMIT_AREA )
	{
		...
		if( d < 3.0f )
		{
			...
			if( d > -3.0f && DotProduct( trace->normal, light->normal ) > 0.999f )
				return 0;
		}
 ...
 */
	...
	/* store direction */
#ifndef BOUNCEGRID_DIRECTIONALITY
	if( !bouncing )
#endif
...
int LightMain( int argc, char **argv )
{
	...
	backSplashFraction = game->backSplashFraction;
	if( backSplashFraction < 0.0f )
		backSplashFraction = 0.0f;
	if( backSplashFraction != 0.0f )
		Sys_Printf( " surfacelight back splash enabled with factor %.4f\n", backSplashFraction );
	else
		Sys_Printf( " surfacelight back splash disabled\n" );
	...
	for( i = 1; i < (argc - 1); i++ )
	{
		...
		/* jal: light backsplash fraction (0 = disabled) */
		else if( !strcmp( argv[ i ], "-backsplashfraction" ) )
		{
			backSplashFraction = atof( argv[ i + 1 ] );
			if( backSplashFraction <= 0.0f )
				backSplashFraction = 0.0f;
			if( backSplashFraction >= 1.0f )
				backSplashFraction = 1.0f;
			Sys_Printf( "light back splash fraction set to %.3f\n", backSplashFraction );
			i++;
		}
	...
	if( IntForKey( &entities[ 0 ], "_lomem" ) != 0 )
	{
		loMem = qtrue;
		Sys_Printf( "Enabling low-memory (potentially slower) lighting mode\n" );
	}
