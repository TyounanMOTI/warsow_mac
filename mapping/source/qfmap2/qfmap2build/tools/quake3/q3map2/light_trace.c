...
/* jal: skip patches and models from lighting grid traces */
//#define GRID_SKIP_BUGGED
...
#define MAX_TW_VERTS			24 // vortex: increased from 12 to 24 for ability to compile some insane maps with large curve count
...
#ifdef GRID_SKIP_BUGGED
		ti.skipGrid = (ds->surfaceType == MST_PATCH); // jal: set patches to skip grid light tracing because it seems to be bugged
#else
		ti.skipGrid = qfalse;
#endif
...
static void PopulateWithPicoModel( int castShadows, picoModel_t *model, m4x4_t transform )
{
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		ti.surfaceNum = -1;
#ifdef GRID_SKIP_BUGGED
		ti.skipGrid = qtrue; // jal: set picomodels to be skept from light tracing when lighting the grid
#else
		ti.skipGrid = qfalse;
#endif
...
qboolean TraceTriangle( traceInfo_t *ti, traceTriangle_t *tt, trace_t *trace )
{
	...
	/* skip marked skipGrid surfaces when lighting the grid */
	if( inGrid && ti->skipGrid )
		return qfalse;
