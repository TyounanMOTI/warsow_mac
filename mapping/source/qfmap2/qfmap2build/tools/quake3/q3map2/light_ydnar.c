			...
			for( y = 0; y < lm->sh; y++ )
			{
				for( x = 0; x < lm->sw; x++ )
				{
...
#endif
					{
						...
						/* jal : add the contribution to the deluxemap */
						if( deluxemap )
						{
...
void IlluminateVertexes2( int num )
{
	int i, lightmapNum;
	bspDrawSurface_t	*ds;
	surfaceInfo_t		*info;
	...
	trace_t				trace;
	bspDrawVert_t		*verts;
	float				*vertLuxel, *radiosityVertLuxel;
	vec3_t				colorContribution[ MAX_LIGHTMAPS ];
	...
	for( i = 0; i < ds->numVerts; i++ )
	{
		...
		if( trace.cluster < 0 )
		{
			...
			{
				for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
					VectorCopy( ambientColor, colorContribution[ lightmapNum ] );
			}
		}
		else
		{
			...
			/* light */
			LightingAtSample( &trace, ds->vertexStyles, colorContribution );
		}

		/* store */
		for( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
		{
			/* jal: add the floodlight */
			VectorAdd( colorContribution[ lightmapNum ], floodColor, colorContribution[ lightmapNum ] );

			/* in surfaces with a lightmap find out the contribution converted from lightmap luxels */
			if( lm != NULL && lm->superLuxels[ lightmapNum ] != NULL && !cpmaHack )
			{
				float samples, *luxel;
				int	x, y, sx, sy, radius, maxRadius, *cluster;
				vec3_t lmcolorContribution[ MAX_LIGHTMAPS ];

				/* get max search radius */
				maxRadius = lm->sw;
				maxRadius = maxRadius > lm->sh ? maxRadius : lm->sh;

				VectorClear( lmcolorContribution[ lightmapNum ] );
				...
				for( radius = 0; radius < maxRadius && samples <= 0.0f; radius++ )
				{
					...
					for( sy = (y - radius); sy <= (y + radius); sy++ )
					{
						...
						for( sx = (x - radius); sx <= (x + radius); sx++ )
						{
							...
							/* add its distinctiveness to our own */
							VectorAdd( lmcolorContribution[ lightmapNum ], luxel, lmcolorContribution[ lightmapNum ] );
							samples += luxel[ 3 ];
						}
					}
				}
				...
				/* any color? */
				if( samples > 0.0f )
					VectorDivide( lmcolorContribution[ lightmapNum ], samples, lmcolorContribution[ lightmapNum ] );

				/* avoid too dark vertexes */
				if( !bouncing )
				{
					vec3_t minFloodLightColor;

					VectorScale( floodlightRGB, floodlightIntensity * 0.4f, minFloodLightColor );

					if( RGBTOGRAY( lmcolorContribution[ lightmapNum ] ) < RGBTOGRAY( ambientColor ) )
						VectorCopy( ambientColor, lmcolorContribution[ lightmapNum ] );

					if( floodlighty && ( RGBTOGRAY( lmcolorContribution[ lightmapNum ] ) < RGBTOGRAY( minFloodLightColor ) ) )
						VectorCopy( minFloodLightColor, lmcolorContribution[ lightmapNum ] );
				}

				/* Use it only if it's brighter than the direct lit contribution */
				if( RGBTOGRAY( lmcolorContribution[ lightmapNum ] ) > RGBTOGRAY( colorContribution[ lightmapNum ] ) )
					VectorCopy( lmcolorContribution[ lightmapNum ], colorContribution[ lightmapNum ] );
			}

			/* override with debugging colors */

			/* color the luxel with the normal? */
			if( normalmap )
			{
				colorContribution[ lightmapNum ][ 0 ] = (verts[ i ].normal[ 0 ] + 1.0f) * 127.5f;
				colorContribution[ lightmapNum ][ 1 ] = (verts[ i ].normal[ 1 ] + 1.0f) * 127.5f;
				colorContribution[ lightmapNum ][ 2 ] = (verts[ i ].normal[ 2 ] + 1.0f) * 127.5f;
			}

			/* color the luxel with surface num? */
			else if( debugSurfaces )
				VectorCopy( debugColors[ num % 12 ], colorContribution[ lightmapNum ] );

			/* copy current contribution for radiosity lighting */
			radiosityVertLuxel = RAD_VERTEX_LUXEL( lightmapNum, ds->firstVert + i );
			VectorCopy( colorContribution[ lightmapNum ], radiosityVertLuxel );

			/* add current contribution to total */
			vertLuxel = VERTEX_LUXEL( lightmapNum, ds->firstVert + i );
			VectorAdd( vertLuxel, colorContribution[ lightmapNum ], vertLuxel );

			/* make sure the color has a minimum brightness */
			if( !bouncing )
			{
				vec3_t minFloodLightColor;

				VectorScale( floodlightRGB, floodlightIntensity * 0.15f, minFloodLightColor );

				if( RGBTOGRAY( vertLuxel ) < RGBTOGRAY( ambientColor ) )
					VectorCopy( ambientColor, vertLuxel );

				if( floodlighty && ( RGBTOGRAY( vertLuxel ) < RGBTOGRAY( minFloodLightColor ) ) )
					VectorCopy( minFloodLightColor, vertLuxel );
			}
...
void SetupFloodLight( void )
{
	...
	/* let floodlight be applied with each bounce pass? */
	if( IntForKey( &entities[ 0 ], "_floodlightmultipass" ) != 0 )
		floodlightMultipass = qtrue;

	/* scale floodlight intensity */
	v1 = FloatForKey( &entities[ 0 ], "_floodlightscale" );
	if( v1 > 0.0f )
		floodlightScale = v1;
}
...
float FloodLightForSample( trace_t *trace , float floodLightDistance, qboolean floodLightLowQuality)
{
	...
	/* return to sender */
	return outLight * floodlightScale;
}
		...
		for( y = 0; y < lm->sh; y++ )
		{
			for( x = 0; x < lm->sw; x++ )
			{
				...
				if (deluxemap && floodlight[3] > 0)
				{
					...
					// jal : use AT LEAST this amount of contribution from ambient for the deluxemap, fixes points that receive ZERO light
					if(brightness < 0.00390625f)
						brightness = 0.00390625f;
